...
| Namespace | Base Controller | Auth | Layout | Audience |
|---|---|---|---|---|
| `/` | `ApplicationController` | `authenticate_user!` | `application` | Drivers |
| `/admin/` | `Admin::BaseController` | `authenticate_admin!` | `admin` | Platform admins |
| `/workshop_management/` | `WorkshopManagement::BaseController` | `authenticate_user!` + `require_workshop_access!` | `workshop` | Workshop owners/staff |

| Model | Key Rule |
|---|---|
| User | Single people model. Role enum (driver only in MVP). Never split into separate models. |
| Admin | Separate Devise model. Never in users table. Never a User. |
| WorkshopOperator | Join table for user ↔ workshop. Access is determined here, not on User#role. |
| ServiceRequest | Price snapshotted at create. Optimistic locking on transitions. |
| ServiceRecord | 1:1 with ServiceRequest. Append-only. Unique DB index. |
| CarTransferEvent | Append-only. No `updated_at`. |
| CarOwnershipRecord | Append-only ownership timeline. |
| Queue | One per workshop/category/day. Composite unique index. |
| QueueEntry | Position immutable. Turbo Stream broadcasts on status change. |
